...
const CrudCuidad = () => {
    ...
    useEffect(() => {
        const ciudService = new ProductService();
        ciudService.getCiudad().then((data) => setProducts(data));
    }, []);
    ...
    const saveProduct = () => {
        ...
        if (product.name.trim()) {
            ...
            if (product.id) {
                ...
                const ciudservice = new ProductService();
                ciudservice.putProvincia(_product); 
                toast.current.show({
                    severity: "success",
                    summary: "Successful",
    ...
    const deleteSelectedProducts = () => {
        ...
        setDeleteProductDialog(_products);
        setDeleteProductsDialog(false);
                ...
                <div className="card">
                    ...
                    <Dialog visible={productDialog} style={{ width: "450px" }} header="Cuidad" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                        ...
                        <div className="field">
                            ...
                            />
                            {submitted && !product.name && <small className="p-invalid">El nombre de la ciudad es requerido.</small>}
                        </div>
                        <div className="field">
                            <label htmlFor="name">Nombre Provincia</label>
                            <InputText
                                id="name"
                                value={product.name}
                                onChange={(e) => onInputChange(e, "name")}
                            ...
                            />
                            {submitted && !product.name && <small className="p-invalid">El nombre de la provincia es requerido.</small>}
                        </div>
                    </Dialog>
